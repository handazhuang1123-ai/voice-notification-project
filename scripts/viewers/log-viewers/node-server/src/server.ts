...
class LogViewerServer {
    ...
    public async start(): Promise<void> {
        try {
            ...
            const url = `http://${this.config.server.host}:${this.config.server.port}/log-viewer/index.html`;
            this.openBrowser(url);

            console.log('\n=== Pip-Boy Log Viewer Ready ===');
            console.log(`  URL: ${url}`);
            console.log('\nPress Ctrl+C to stop the server and exit...');
            console.log(`The server will serve files from: ${this.config.paths.viewerRoot}`);
        } catch (error) {
