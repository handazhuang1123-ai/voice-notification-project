        ...
        try {
            ...
            const response = await axios.post(`${this.ollamaUrl}/api/generate`, {
                ...
                options: {
                    ...
                    num_ctx: options.num_ctx || 8192,  // 上下文窗口
...
- 确保 JSON 格式正确，可以被解析`;
    ...
    async checkHealth() {
        try {
            ...
            if (!hasModel) {
                console.warn(`⚠️ 模型 ${this.model} 未找到，可用模型:`, models.map(m => m.name));
                return false;
            }
            ...
            console.error('❌ Ollama 服务不可用:', error.message);
            return false;
        }
    }
